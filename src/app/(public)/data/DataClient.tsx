...
const PEMANFAATAN_OPTS = ["Sawah", "Tegalan", "Kebun", "Rumah Warga", "Taman Desa", "Pos Kamling", "Balai Warga", "Rumah Pensiunan"];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {entries.map(([nama, luas]) => {
        ...
        return (
          <div key={nama} style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{
              width: 130, fontSize: 13, fontWeight: 600,
              textAlign: "right", color: "var(--navy-800)",
              fontFamily: '"Manrope", sans-serif', flexShrink: 0,
            }}>
              {nama}
            </div>
            <div style={{ flex: 1, height: 14, background: "rgba(160,125,47,.2)", borderRadius: 4, overflow: "hidden" }}>
              <div style={{
                height: "100%",
                width: `${w}%`,
            ...
            <div style={{
              width: 72, fontFamily: '"Cormorant Garamond", serif',
...
function DonutChart({ data }: { data: TkdPublicRow[] }) {
  ...
  const segments = [
    ...
    ...s,
    luas: data.filter((r) => r.jenisTanah === s.key).reduce((a, r) => a + r.luasM2, 0),
  }));
    ...
    <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
      <div style={{ width: 120, height: 120, flexShrink: 0 }}>
        <svg viewBox="0 0 120 120" width="120" height="120">
          ...
          <circle cx={cx} cy={cy} r={22} fill="#f3e9cf" stroke="#c49a45" strokeWidth={1.5} />
        </svg>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6, fontFamily: '"Cormorant Garamond", serif', fontSize: 14 }}>
        <div style={{ fontFamily: '"Cinzel", serif', fontSize: 11, letterSpacing: "2px", color: "var(--navy-800)", fontWeight: 600, marginBottom: 4 }}>
          JENIS PENGGUNAAN
        </div>
        {arcs.map((a) => (
          <div key={a.key} style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--ink-soft)" }}>
            <span style={{
              width: 11, height: 11, borderRadius: "50%", background: a.color,
              display: "inline-block", boxShadow: "0 0 0 1.5px var(--cream-50), 0 0 0 2.5px rgba(0,0,0,.15)",
            }} />
...
export function DataClient({ padukuhanOptions }: Props) {
  ...
  const filtered = useMemo(() => {
    return rawData.filter((r) => {
      ...
      if (filterPemanfaatan && r.pemanfaatan !== filterPemanfaatan) return false;
      return true;
    });
  }, [rawData, filterPadukuhan, filterJenis, filterPenggunaan, filterPemanfaatan]);
  ...
  const luasByJenis = (jenis: string) =>
    filtered.filter((r) => r.jenisTanah === jenis).reduce((s, r) => s + r.luasM2, 0);
  ...
  if (isLoading) {
    return (
      <div className="animate-fadeUp text-center" style={{ padding: "64px 0", color: "var(--navy-800)", fontStyle: "italic" }}>
  ...
  return (
    <div className="animate-fadeUp">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          >
            <div className="stat-card-heritage" style={{ textAlign: "center" }}>
              <div style={{ fontFamily: '"Cinzel", serif', fontSize: 10, letterSpacing: "2px", color: "var(--navy-800)", fontWeight: 600, marginBottom: 6 }}>TOTAL LUASAN</div>
              <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 26, fontWeight: 700, color: "var(--navy-900)", lineHeight: 1 }}>
                {allTotalLuas >= 10000
            ...
            <div className="stat-card-heritage" style={{ textAlign: "center" }}>
              <div style={{ fontFamily: '"Cinzel", serif', fontSize: 10, letterSpacing: "2px", color: "var(--navy-800)", fontWeight: 600, marginBottom: 6 }}>TOTAL BIDANG</div>
              <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 26, fontWeight: 700, color: "var(--navy-900)", lineHeight: 1 }}>
                {allTotalBidang}
              </div>
            </div>
            <div className="stat-card-heritage" style={{ textAlign: "center" }}>
              <div style={{ fontFamily: '"Cinzel", serif', fontSize: 10, letterSpacing: "2px", color: "var(--navy-800)", fontWeight: 600, marginBottom: 6 }}>PADUKUHAN</div>
              <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 26, fontWeight: 700, color: "var(--navy-900)", lineHeight: 1 }}>
                {allPadukuhan}
              </div>
            </div>
      ...
      {/* Divider */}
      <div style={{ borderTop: "1px solid var(--gold-500)", marginBottom: 28, position: "relative" }}>
        <span style={{
          position: "absolute", right: 0, top: -10,
          color: "var(--gold-500)", background: "var(--cream-50)",
      ...
      <div
        style={{
          ...
          gap: 20,
          background: "linear-gradient(180deg, rgba(255,248,210,.55), rgba(214,196,147,.3))",
        ...
        className="max-[768px]:grid-cols-2 max-[480px]:grid-cols-1"
      >
        {/* Padukuhan */}
        <div>
          <label className="label-heritage" htmlFor="f-padukuhan">Filter Padukuhan</label>
          <select
            id="f-padukuhan"
            className="select-heritage"
            value={filterPadukuhan}
            onChange={(e) => { setFilterPadukuhan(e.target.value); handleFilterChange(); }}
          >
        ...
        {/* Jenis */}
        <div>
          <label className="label-heritage" htmlFor="f-jenis">Jenis Tanah Kalurahan</label>
          <select
            id="f-jenis"
            className="select-heritage"
            value={filterJenis}
        ...
        {/* Penggunaan */}
        <div>
          <label className="label-heritage" htmlFor="f-penggunaan">Penggunaan Tanah</label>
          <select
            id="f-penggunaan"
            className="select-heritage"
            value={filterPenggunaan}
            onChange={(e) => { setFilterPenggunaan(e.target.value); handleFilterChange(); }}
          >
            <option value="">Penggunaan Tanah</option>
            {PENGGUNAAN_OPTS.map((o) => <option key={o} value={o}>{o}</option>)}
          </select>
        </div>

        {/* Pemanfaatan */}
        <div>
          <label className="label-heritage" htmlFor="f-pemanfaatan">Pemanfaatan Tanah</label>
          <select
            id="f-pemanfaatan"
            className="select-heritage"
            value={filterPemanfaatan}
            onChange={(e) => { setFilterPemanfaatan(e.target.value); handleFilterChange(); }}
          >
            <option value="">Pemanfaatan Tanah</option>
            {PEMANFAATAN_OPTS.map((o) => <option key={o} value={o}>{o}</option>)}
          </select>
        </div>
      </div>
      ...
      <div
        ...
        className="max-[960px]:grid-cols-2 max-[480px]:grid-cols-1"
      >
        ...
        >
          <div style={{ fontFamily: '"Cinzel", serif', fontSize: 11, letterSpacing: "2px", color: "var(--navy-800)", fontWeight: 600, marginBottom: 6 }}>
            TOTAL TANAH KALURAHAN
          </div>
          <div style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 42, fontWeight: 700, color: "var(--navy-900)", lineHeight: 1 }}>
            {fmtLuas(totalLuas)}
          </div>
          <div style={{ fontSize: 14, color: "var(--ink-soft)", fontStyle: "italic", marginTop: 6 }}>
            dari {filtered.length} bidang tanah
          </div>
        </div>

        {/* Double col: Kas + Pengarem */}
        <div style={{ gridRow: "span 2", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <table className="tbl-heritage" style={{ fontSize: 17 }}>
            <thead>
              <tr>
                ...
                <th style={{ textAlign: "left", paddingLeft: 16 }}>Padukuhan</th>
                <th style={{ textAlign: "left", paddingLeft: 16 }}>Nama Bidang</th>
                <th>Penggunaan</th>
                <th>Pemanfaatan</th>
                <th>Jenis</th>
                <th>Luas (m²)</th>
            ...
            <tbody>
              {pageData.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: "40px 0", textAlign: "center", color: "var(--ink-soft)", fontStyle: "italic" }}>
                    Tidak ada data yang sesuai filter.
                  </td>
                </tr>
              ) : (
                pageData.map((row, i) => {
                  const badge = BADGE_CONFIG[row.jenisTanah] || BADGE_CONFIG.LAINNYA;
                  return (
                    <tr key={row.id}>
                      <td style={{ fontWeight: 600, color: "var(--navy-800)" }}>
                        {(page - 1) * PAGE_SIZE + i + 1}
                      </td>
                      <td style={{ textAlign: "left", paddingLeft: 16 }}>{row.padukuhan}</td>
                      <td style={{ textAlign: "left", paddingLeft: 16 }}>{row.nama}</td>
