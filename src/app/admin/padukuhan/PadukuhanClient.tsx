...
export function PadukuhanClient({
  initialData,
}: {
  initialData: PadukuhanWithCount[];
}) {
  const [isRefreshing, setIsRefreshing] = useState(false);

  const { data, refetch, isLoading } = trpc.padukuhan.listAdmin.useQuery(
    undefined,
    {
      initialData,
    },
            ...
            <tbody>
              {pageData.map((item, idx) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-bold ${item.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                    >
              ...
              >
                <div>
                  <label className="label-heritage">
                    Nama Padukuhan <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    required
